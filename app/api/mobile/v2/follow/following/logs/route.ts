// app/api/mobile/v2/follow/following/logs/route.ts
import { NextResponse } from "next/server";
import { withAuth } from "@/lib/apiHelpers";
import { getFollowingLogsV2 } from "@/server/follow/follow.service";
import { ServiceError } from "@/server/common/errors";

// GET /api/mobile/v2/follow/following/logs?relationshipId=...&profileId=...&startDate=...&endDate=...
// Get medication logs with full medicine names and note
export async function GET(request: Request) {
    return withAuth(request, async ({ prismaUser }) => {
        try {
            ...
            const result = await getFollowingLogsV2({
