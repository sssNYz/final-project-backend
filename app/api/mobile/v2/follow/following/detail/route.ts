// app/api/mobile/v2/follow/following/detail/route.ts
import { NextResponse } from "next/server";
import { withAuth } from "@/lib/apiHelpers";
import { getFollowingDetailV2 } from "@/server/follow/follow.service";
import { ServiceError } from "@/server/common/errors";

// GET /api/mobile/v2/follow/following/detail?relationshipId=...
// Get following detail (shows ownerNickname/ownerPicture set by viewer)
export async function GET(request: Request) {
    return withAuth(request, async ({ prismaUser }) => {
        try {
            ...
            const result = await getFollowingDetailV2({
