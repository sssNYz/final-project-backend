// app/api/mobile/v1/medicine/list/route.ts
import { NextRequest, NextResponse } from "next/server";
import { MedicineType } from "@prisma/client";

import { requireAuth } from "@/lib/auth";
import { ServiceError } from "@/server/common/errors";
import { listMedicinesForUser } from "@/server/medicine/medicine.service";
...
function toErrorResponse(error: unknown) {
  ...
  console.error("Error in list medicines for user:", error);
  return {
    status: 500,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await listMedicinesForUser({
      search,
      type,
      page,
      pageSize,
